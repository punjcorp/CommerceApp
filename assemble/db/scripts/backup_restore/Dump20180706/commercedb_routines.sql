...
--
-- Temporary table structure for view `v_location_sales_data`
--

DROP TABLE IF EXISTS `v_location_sales_data`;
/*!50001 DROP VIEW IF EXISTS `v_location_sales_data`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `v_location_sales_data` AS SELECT 
 1 AS `location_id`,
 1 AS `business_date`,
 1 AS `basket_size`,
 1 AS `basket_amount`,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_location_sales_data` AS select `t1`.`location_id` AS `location_id`,`t1`.`business_date` AS `business_date`,`t1`.`basket_size` AS `basket_size`,`t1`.`basket_amount` AS `basket_amount`,`t1`.`txn_count` AS `txn_count`,`t1`.`revenue` AS `revenue`,`t2`.`customer_count` AS `customer_count`,`t3`.`discount_percent` AS `discount_percent`,`t4`.`discount_amount` AS `discount_amount` from ((((select `txn_basket`.`location_id` AS `location_id`,`txn_basket`.`business_date` AS `business_date`,round((sum(`txn_basket`.`txn_item_count`) / count(`txn_basket`.`txn_no`)),2) AS `basket_size`,round((sum(`txn_basket`.`basket_amounts`) / count(`txn_basket`.`txn_no`)),2) AS `basket_amount`,count(`txn_basket`.`txn_no`) AS `txn_count`,round(sum(`txn_basket`.`gross_amounts`),2) AS `revenue` from (select `txn_itm`.`location_id` AS `location_id`,`txn_itm`.`business_date` AS `business_date`,`txn_itm`.`register` AS `register`,`txn_itm`.`txn_no` AS `txn_no`,sum(`txn_itm`.`gross_qty`) AS `txn_item_count`,sum(`txn_itm`.`net_amount`) AS `basket_amounts`,sum(`txn_itm`.`gross_amount`) AS `gross_amounts` from (`commercedb`.`txn_master` `txn` join `commercedb`.`txn_li_item` `txn_itm`) where ((`txn`.`txn_type` in ('SALE','RETURN')) and (`txn`.`status` = 'COMPLETED') and (`txn`.`business_date` = `txn_itm`.`business_date`) and (`txn`.`register` = `txn_itm`.`register`) and (`txn`.`location_id` = `txn_itm`.`location_id`) and (`txn`.`txn_no` = `txn_itm`.`txn_no`)) group by `txn_itm`.`location_id`,`txn_itm`.`business_date`,`txn_itm`.`register`,`txn_itm`.`txn_no`) `txn_basket` group by `txn_basket`.`location_id`,`txn_basket`.`business_date`) `t1` join (select `commercedb`.`txn_customer`.`location_id` AS `location_id`,`commercedb`.`txn_customer`.`business_date` AS `business_date`,count(0) AS `customer_count` from `commercedb`.`txn_customer` group by `commercedb`.`txn_customer`.`location_id`,`commercedb`.`txn_customer`.`business_date`) `t2`) join (select `txn_discount_percent`.`location_id` AS `location_id`,`txn_discount_percent`.`business_date` AS `business_date`,sum(`txn_discount_percent`.`total_discount`) AS `sum(total_discount)`,sum(`txn_discount_percent`.`total_price`) AS `sum(total_price)`,abs(round(((sum(`txn_discount_percent`.`total_discount`) / sum(`txn_discount_percent`.`total_price`)) * 100),2)) AS `discount_percent` from (select `txn_itm`.`location_id` AS `location_id`,`txn_itm`.`business_date` AS `business_date`,`txn_itm`.`register` AS `register`,`txn_itm`.`txn_no` AS `txn_no`,sum(`txn_itm`.`discount_amount`) AS `total_discount`,sum(`txn_itm`.`extended_amount`) AS `total_price` from (`commercedb`.`txn_master` `txn` join `commercedb`.`txn_li_item` `txn_itm`) where ((`txn`.`txn_type` in ('SALE','RETURN')) and (`txn`.`status` = 'COMPLETED') and (`txn`.`business_date` = `txn_itm`.`business_date`) and (`txn`.`register` = `txn_itm`.`register`) and (`txn`.`location_id` = `txn_itm`.`location_id`) and (`txn`.`txn_no` = `txn_itm`.`txn_no`) and (`txn_itm`.`discount_percentage` is not null)) group by `txn_itm`.`location_id`,`txn_itm`.`business_date`,`txn_itm`.`register`,`txn_itm`.`txn_no`) `txn_discount_percent` group by `txn_discount_percent`.`location_id`,`txn_discount_percent`.`business_date`) `t3`) join (select `txn_discount_percent`.`location_id` AS `location_id`,`txn_discount_percent`.`business_date` AS `business_date`,round(sum(`txn_discount_percent`.`total_discount`),2) AS `discount_amount` from (select `txn_itm`.`location_id` AS `location_id`,`txn_itm`.`business_date` AS `business_date`,`txn_itm`.`register` AS `register`,`txn_itm`.`txn_no` AS `txn_no`,sum(`txn_itm`.`discount_amount`) AS `total_discount` from (`commercedb`.`txn_master` `txn` join `commercedb`.`txn_li_item` `txn_itm`) where ((`txn`.`txn_type` in ('SALE','RETURN')) and (`txn`.`status` = 'COMPLETED') and (`txn`.`business_date` = `txn_itm`.`business_date`) and (`txn`.`register` = `txn_itm`.`register`) and (`txn`.`location_id` = `txn_itm`.`location_id`) and (`txn`.`txn_no` = `txn_itm`.`txn_no`) and isnull(`txn_itm`.`discount_percentage`) and (`txn_itm`.`discount_amount` is not null)) group by `txn_itm`.`location_id`,`txn_itm`.`business_date`,`txn_itm`.`register`,`txn_itm`.`txn_no`) `txn_discount_percent` group by `txn_discount_percent`.`location_id`,`txn_discount_percent`.`business_date`) `t4`) where ((`t1`.`location_id` = `t2`.`location_id`) and (`t1`.`business_date` = `t2`.`business_date`) and (`t3`.`location_id` = `t2`.`location_id`) and (`t3`.`business_date` = `t2`.`business_date`) and (`t3`.`location_id` = `t4`.`location_id`) and (`t3`.`business_date` = `t4`.`business_date`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `p_get_current_item_price`(IN i_item_id bigint, IN i_location_id int, OUT o_item_price_id bigint)
begin
...
-- Dump completed on 2018-07-06 19:32:36
