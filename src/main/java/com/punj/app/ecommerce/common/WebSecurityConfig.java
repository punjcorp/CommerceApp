...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/", "/save_pi_pos_setup", "/resources/**", "/webjars/**", "/js/**", "/css/**", "/images/**").permitAll().antMatchers("/admin").hasRole("ADMIN")
				.antMatchers("/pos").access("hasRole('ADMIN')  or hasRole('CASHIER')").anyRequest().authenticated().and().formLogin().loginPage("/start").failureUrl("/start?error")
				.permitAll().and().logout().logoutRequestMatcher(new AntPathRequestMatcher("/logout")).logoutSuccessUrl("/start");
