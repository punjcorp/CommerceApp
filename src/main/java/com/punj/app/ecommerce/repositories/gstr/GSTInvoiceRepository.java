...
public interface GSTInvoiceRepository extends JpaRepository<GSTInvoice, GSTInvoiceId> {
	...
	@Query(value = "SELECT * FROM commercedb.v_gstr_one_invoice where location_id=?1 and business_date between ?2 and ?3 ORDER BY gst_no ASC, location_id ASC , business_date ASC , register ASC , txn_no ASC , created_date ASC , rate ASC", nativeQuery = true)
