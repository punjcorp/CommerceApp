...
public class DailyDeedServiceImpl implements DailyDeedService {
	...
	/**
	 * @param locationStatusRepository the locationStatusRepository to set
	 */
	@Autowired
	...
	/**
	 * @param tenderCountRepository the tenderCountRepository to set
	 */
	@Autowired
	...
	public Boolean saveStoreOpenTxn(DailyTransaction storeOpenDetails, String username) {
		...
		Transaction txnDetails = this.transactionService.createTransactionInstance(txnIdDTO, username,
		...
		if (txnDetails != null) {
			List<TenderCount> tenderCountList = TenderCountDTOConverter.transformTenderList(
					storeOpenDetails.getTenders(), txnDetails.getTransactionId(), username,
					ServiceConstants.TXN_OPEN_STORE);
	...
	/**
	 * This method is used to perform the following tasks 1. Open the repository for
	 * the day
	 * 
	 */
	private void updateDailySafe(Transaction txnDetails, List<TenderCount> tenderCountList, String username) {

		List<LocationSafe> locSafeList = this.financeService
				.retrieveAllSafes(txnDetails.getTransactionId().getLocationId());
	...
	/**
	 * This method is used to perform the following tasks 1. Update the store
	 * /register totals as applicable
	 */
	...
	public DailyDeedDTO saveRegisterOpenTxn(DailyTransaction registerOpenDetails, String username) {
		...
		Transaction txnDetails = this.transactionService.createTransactionInstance(txnIdDTO, username,
				ServiceConstants.TXN_OPEN_REGISTER);
		DailyTotals registerDailyTotals = this.saveTransactionTenderDetails(txnDetails, registerOpenDetails, username,
				ServiceConstants.TXN_OPEN_REGISTER);
	...
	public DailyDeedDTO saveRegisterCloseTxn(DailyTransaction registerCloseDetails, String username) {
		...
		Transaction txnDetails = this.transactionService.createTransactionInstance(txnIdDTO, username,
				ServiceConstants.TXN_CLOSE_REGISTER);
		DailyTotals registerDailyTotals = this.saveTransactionTenderDetails(txnDetails, registerCloseDetails, username,
				ServiceConstants.TXN_CLOSE_REGISTER);
		if (registerDailyTotals != null) {
			DailyDeedDTO dailyDeedDTO = new DailyDeedDTO();
			dailyDeedDTO.setDailyTotals(registerDailyTotals);
	...
	public DailyDeedDTO saveStoreCloseTxn(DailyTransaction storeCloseDetails, String username) {
		...
		Transaction txnDetails = this.transactionService.createTransactionInstance(txnIdDTO, username,
				ServiceConstants.TXN_CLOSE_STORE);
		DailyTotals storeDailyTotals = this.saveTransactionTenderDetails(txnDetails, storeCloseDetails, username,
				ServiceConstants.TXN_CLOSE_STORE);
	...
	private DailyTotals saveTransactionTenderDetails(Transaction txnDetails, DailyTransaction txnRawDetails,
			String username, String txnType) {
		DailyTotals resultDailyTotals = null;
		txnDetails = this.transactionService.saveTransaction(txnDetails);
		if (txnDetails != null) {
			List<TenderCount> tenderCountList = TenderCountDTOConverter.transformTenderList(txnRawDetails.getTenders(),
					txnDetails.getTransactionId(), username, txnType);
			tenderCountList = this.tenderCountRepository.save(tenderCountList);
			if (tenderCountList != null && !tenderCountList.isEmpty()) {
				...
				Integer tenderId = null;
				for (TenderCount tenderCount : tenderCountList) {
					if (ServiceConstants.TXN_CLOSE_REGISTER.equals(txnType)
							|| ServiceConstants.TXN_CLOSE_STORE.equals(txnType)) {
				...
				LocationSafe locationSafe = this.retrieveStoreSafe(txnDetails.getTransactionId().getLocationId(),
						tenderId);
	...
	@Override
	public Map<Integer, List<TenderCount>> retrieveTenderCounts(Integer locationId, LocalDateTime businessDate,
			String txnType) {
	...
	public RegisterDTO retrieveRegisterConcilationDtls(Integer locationId, LocalDateTime businessDate, String txnType) {
		...
		if (registerDTO != null) {
			...
			Map<Integer, List<TenderCount>> tenderCountMap = this.retrieveTenderCounts(locationId, businessDate,
					txnType);
	...
	private List<LocationStatus> getLocationStausForOpening(Integer locationId, LocalDateTime businessDate) {
		List<LocationStatus> locStatusList = this.locationStatusRepository.getLocationStatusSinceDate(locationId,
	...
	public void resetLocationForBusiness(Integer locationId, LocalDateTime businessDate) {
		...
		logger.info("The last register close and store close transactions has been retrieved for {} business date",
				businessDate);
		if (storeCloseTxn != null && regCloseTxn != null) {
			...
			logger.info(
					"The last register close and store close transactions for {} business date has been delete successfully ",
					businessDate);
	...
	public Boolean skipOpenProcess(DailyTransaction storeOpenDetails, String username) {
		...
		Transaction storeOpenTxn = this.transactionService.searchLocationOpenTxn(
				storeOpenDetails.getTransactionId().getLocationId(),
				storeOpenDetails.getTransactionId().getBusinessDate());
		...
		if (storeOpenTxn != null) {
			...
			Transaction regTxnDetails = this.transactionService.createTransactionInstance(txnIdDTO, username,
					ServiceConstants.TXN_OPEN_REGISTER);
