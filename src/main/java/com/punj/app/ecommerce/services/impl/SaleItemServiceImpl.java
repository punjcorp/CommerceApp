		...
		if (item != null) {
			...
			if (itemStock != null) {
				...
				if (StringUtils.isNotBlank(gstFlag)) {
					if (gstFlag.equals(ServiceConstants.TAX_GST_FLAG_SGST) || gstFlag.equals(ServiceConstants.TAX_GST_FLAG_IGST)) {
						taxList = this.retrieveTax(locationId, item.getItemOptions().getTaxGroupId(), ServiceConstants.TAX_WITHIN_STATE);
						if(taxList!=null)
							taxList.addAll(this.retrieveTax(locationId, item.getItemOptions().getTaxGroupId(), ServiceConstants.TAX_OTHER_STATE));
