...
public class ManageOrderController {
	...
	private void updateModelBeans(Model model, OrderBeansDTO orderBeanDTO, SearchBean searchBean) {
		...
		Pager tmpPager = orderBeanDTO.getPager();
	...
	public String deleteOrders(@ModelAttribute OrderBeansDTO orders, RedirectAttributes redirectAttrs, final HttpServletRequest req, Locale locale,
			Authentication authentication) {
		try {
			UserDetails userDetails = (UserDetails) authentication.getPrincipal();
			List<BigInteger> orderIds = OrderTransformer.retrieveEligibleOrders(orders);
