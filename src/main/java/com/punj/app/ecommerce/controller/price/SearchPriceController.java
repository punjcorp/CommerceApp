...
public class SearchPriceController {
	...
	public String searchPrices(@ModelAttribute PriceBeanDTO priceBeanDTO, Authentication authentication, Model model, Locale locale) {
		try {
			UserDetails userDetails = (UserDetails) authentication.getPrincipal();
			if (userDetails != null) {
				List<ItemPrice> itemPriceList = this.priceService.searchItemPrice(priceBeanDTO.getItemId(), priceBeanDTO.getLocationId(),
						priceBeanDTO.getPriceType());
				List<PriceBean> priceBeans = PriceTransformer.transformItemPriceList(itemPriceList);
				priceBeanDTO.setPriceBeans(priceBeans);
				this.updatePriceBeanDTO(priceBeanDTO, model);
				this.updateSearchStatus(model, priceBeans, locale);
