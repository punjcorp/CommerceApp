...
$(function() {
	...
	$('#btnCloseRegister').click(function() {
		window.location.href = register_close_url;
	});
...
function processRegisterClosure(){
	$('#alertModal').modal('hide');
	$('#screenBusyModal').modal({backdrop: 'static', keyboard: false});
	var formdata = $('#closeRegisterForm').serialize();
	$.post({
		url : "/pos/close_register",
		data : formdata,
...
function viewStoreClose(){
    window.location.href = store_close_url+"="+ closing_location_id;
}