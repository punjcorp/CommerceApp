/**
 * This file contains all the classes needed for a simple Sales transaction
 */
 ...
 */
var SaleLineItem = function(itemId, hsnNo, itemName, itemDesc, qty, price, unitPrice, suggestedPrice, maxRetailPrice, discount, cgstCode, sgstCode, igstCode,
		cgstTax, sgstTax, igstTax, cgstTaxRate, sgstTaxRate, igstTaxRate, itemTotal, itemImage) {
	if (arguments.length > 0) {
		...
		this.cgstCode = cgstCode, this.sgstCode = sgstCode, this.igstCode = igstCode, this.cgstTaxRate = cgstTaxRate;
		...
		this.maxRetailPrice;
		this.discount;
		this.cgstCode, this.sgstCode, this.igstCode, this.cgstTax;
		this.sgstTax;
		this.igstTax;
		this.cgstTaxRate;
		this.sgstTaxRate;
...
$.extend(SaleLineItem.prototype, {
	...
	renderSaleLineItem : function(saleLineItem, viewType, isIGSTFlag) {
		...
		if (isGSTFlag == 'S') {
			...
			compactIgstTaxAmt = '<input id="li_uh_igstRate' + saleLineItem.itemId + '" type="hidden" value="';
	...
	bindKeysForLineItem : function(saleLineItem, viewType) {
		...
		Mousetrap(itemQtyCntl).bind('right', function() {
			itemDiscountCntl.focus();
		});
		Mousetrap(itemQtyCntl).bind('left', function() {
			txnAction.focusPrevious(saleLineItem.itemId);
		});

		Mousetrap(itemDiscountCntl).bind('left', function() {
			itemQtyCntl.focus();
		});
		Mousetrap(itemDiscountCntl).bind('right', function() {
			itemDiscountTypeCntl.focus();
		});
		...
		Mousetrap(itemDeleteBtnCntl).bind('left', function() {
			itemDiscountTypeCntl.focus();
		});
		Mousetrap(itemDeleteBtnCntl).bind('right', function() {
			txnAction.focusNext(saleLineItem.itemId);
		});
		...
		if (viewType == 'DETAILED') {
			...
			Mousetrap(itemQtyCntl).bind('right', function() {
				itemUnitPriceCntl.focus();
			});

			Mousetrap(itemUnitPriceCntl).bind('left', function() {
				itemQtyCntl.focus();
			});
			Mousetrap(itemUnitPriceCntl).bind('right', function() {
				itemSuggestedPriceCntl.focus();
			});

			Mousetrap(itemSuggestedPriceCntl).bind('left', function() {
				itemUnitPriceCntl.focus();
			});
			Mousetrap(itemSuggestedPriceCntl).bind('right', function() {
				itemMRPCntl.focus();
			});

			Mousetrap(itemMRPCntl).bind('left', function() {
				itemSuggestedPriceCntl.focus();
			});
			Mousetrap(itemMRPCntl).bind('right', function() {
				itemDiscountCntl.focus();
			});
	...
	renderReturnLineItem : function(saleLineItem, viewType) {
		...
		suggestedPriceAmt += '<label><small> <span>' + i18next.t('sale_txn_lbl_suggested_price') + '</span></small> </label><br />';
		suggestedPriceAmt += '<div class="input-group text-left">';
		suggestedPriceAmt += '<input class="form-control form-control-sm  pos-amount" id="li_suggestedPriceAmt' + saleLineItem.itemId
				+ '" type="number" min="0" step="0.01" value="';
		suggestedPriceAmt += saleLineItem.suggestedPrice.toFixed(2);
		suggestedPriceAmt += '" ></input></div></div>';
		suggestedPriceAmt += '<input id="li_uh_suggestedPriceAmt' + saleLineItem.itemId + '" type="hidden" value="';
		...
		maxRetailPriceAmt += '<label><small> <span>' + i18next.t('sale_txn_lbl_mrp') + '</span></small> </label><br />';
		maxRetailPriceAmt += '<div class="input-group text-left">';
		maxRetailPriceAmt += '<input class="form-control form-control-sm  pos-amount" id="li_maxRetailPriceAmt' + saleLineItem.itemId
				+ '" type="number" min="0" step="0.01" value="';
		maxRetailPriceAmt += saleLineItem.maxRetailPrice.toFixed(2);
		maxRetailPriceAmt += '"></input></div></div>';
		maxRetailPriceAmt += '<input id="li_uh_maxRetailPriceAmt' + saleLineItem.itemId + '" type="hidden" value="';
		...
		if (viewType == 'COMPACT') {
			...
			finalReturnItemHtml += saleLineItemHtml + qty + compactUnitPriceAmt + compactSuggestedPriceAmt + compactMaxRetailPriceAmt;
			if(isGSTFlag=='S'){
				finalReturnItemHtml +=compactSgstTaxAmt + compactCgstTaxAmt + priceAmt + discountAmt + itemTaxAmt + total; 
			}else if(isGSTFlag=='I'){
				finalReturnItemHtml +=compactIgstTaxAmt + priceAmt + discountAmt + itemTaxAmt + total;
			}else if(isIGSTFlag=='N'){
				finalReturnItemHtml +=priceAmt + discountAmt + total;
			}
			...
			finalReturnItemHtml += outerContainer + firstRowStarts;
			finalReturnItemHtml += saleLineItemHtml + qty + unitPriceAmt + suggestedPriceAmt + maxRetailPriceAmt;
			
			if(isGSTFlag=='S'){
				finalReturnItemHtml += sgstTaxAmt + cgstTaxAmt;
				finalReturnItemHtml += firstRowEnds +secondRowStarts;
				finalReturnItemHtml += priceAmt + discountAmt + itemTaxAmt + total;
			}else if(isGSTFlag=='I'){
				finalReturnItemHtml += igstTaxAmt;
				finalReturnItemHtml += firstRowEnds +secondRowStarts;
				finalReturnItemHtml += priceAmt + discountAmt + itemTaxAmt + total;
			}else if(isIGSTFlag=='N'){
...
$.extend(TenderLineItem.prototype, {
	...
	bindKeysForTenderLineItem : function(controlIndex) {
		...
		Mousetrap(deleteTenderCntl).bind('up', function() {
			txnAction.focusPreviousTender(controlIndex);
		});
		Mousetrap(deleteTenderCntl).bind('down', function() {
			txnAction.focusNextTender(controlIndex);
		});
